...
intersection (Node x ox s) (Node y oy t)
 | x == y = Node x num (intersection s (Node y oy t))
...
La primera representación mantiene el principio de invariante a ordenanza lo que simplifica 
la mayoría de las operaciones que se encuentran dentro del TAD haciéndolo más eficiente.
-}
