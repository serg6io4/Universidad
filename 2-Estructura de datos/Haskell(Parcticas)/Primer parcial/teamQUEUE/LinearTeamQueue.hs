...
enqueue e n Empty = (Node e Empty)
enqueue e n (Node f q)
  | (mod e n) == (mod f n) = (Node f (Node e q))
  | otherwise = (Node f (enqueue e n q))
...
mkTQ e l =  foldl (\q x -> enqueue x e q) Empty l
...
foldrTQ f b Empty = b
foldrTQ f b (Node x q) = f x (foldrTQ f b q)
...
filterTQ p q = foldrTQ (\x acc -> if p x then Node x acc else acc) Empty q
...
dequeueTeam n r@(Node x q)
  | (mod x n == 1) = dequeueTeam n q
  | otherwise = (Node x (dequeueTeam n q))
