...
-- O(1)
isFirst :: Dock a -> Bool
isFirst (D xs ys) = (null xs) || (null (tail xs))
--Porque si el dock es el primero debe quedar como único en el primer array o no haber nada
{-
...
isLast :: Dock a -> Bool
isLast (D xs []) = null xs
isLast (D xs ys) = null ys

-- si ambas están vacías es true o el destacado es el primero de la lista y la otra está vacía es true
{-
