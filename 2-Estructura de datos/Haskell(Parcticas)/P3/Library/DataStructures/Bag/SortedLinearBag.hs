...
delete :: (Ord a) => a -> Bag a -> Bag a
delete _ Empty = Empty  -- No se puede eliminar nada de una bolsa vacía
...
difference (Node x ox s) (Node y oy t)
  ...
  | x > y =(difference (Node x ox s) t)
   where resolucion = if (ox <= oy) then (difference s t) else (Node x (ox-oy) (difference s t))
...
La versión ordenada es más eficiente por el simple hecho que no hace falta
estar buscando datos dentro de toda la bolsa ya que por ejemplo si es un valor pequeño
estará de los primeros y si es de los más grandes estará en los últimos.
